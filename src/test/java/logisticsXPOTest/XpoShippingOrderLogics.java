...
public class XpoShippingOrderLogics extends BrowserConfigGUI {
	...
	public XpoShippingOrderLogics(String JiraUrl, String username, String password, String project) {
	...
	public Issue createJiraIssue(String issueType, String summary, String description, String reporterName,
		...
		try {
			...
			log.info("********************************************");
			log.info("Xpo New issue created in Jira with ID: " + newIssue);
			log.info("Xpo New issue URL is :" + JiraUrl + "/browse/" + newIssue);
			log.info("Xpo issuetype: " + newIssue.getIssueType());
			log.info("Xpo Status:" + newIssue.getStatus());
			log.info("*******************************************");
	...
	public List<String> getAttachments(Issue newIssue) throws JiraException, InterruptedException, IOException {
		...
		log.info("Xpo attachments issuedetails:" + issue.toString());
		List<Attachment> attachments = issue.getAttachments();
		for (Attachment attach : attachments) {
			filename = attach.getFileName();
			log.info("Xpo attachments Filename :" + filename);
	...
	public void getStatus(Issue newIssue) throws JiraException, InterruptedException {
		Issue issue = Jira.getIssue(newIssue.getKey());
		log.info("Xpo status issueid:" + issue.toString());
		log.info("Xpo Status:" + issue.getStatus());
	}

	public Issue getComments(Issue newIssue) throws Exception {

		Issue issue = Jira.getIssue(newIssue.getKey());
		// log.info("issuedetails:" + issue.toString());
		try {
			List<Comment> comments = issue.getComments();
			// if (issue.getStatus()==Investigate)
			for (Comment statement : comments) {
				log.info("Xpo Comments is" + "\n" + statement.getBody());
			}
		} catch (Exception e) {
	...
	public Issue getIssueLinks(Issue newIssue) throws Exception {
		Issue outwardIssue;
		Issue issue = Jira.getIssue(newIssue.getKey());
		log.info("Xpo issulelinks issueid:" + issue.toString());
		try {
			List<IssueLink> issueLinks = issue.getIssueLinks();
			if (issueLinks.isEmpty())
				// log.info("Comments" +comments);
				log.info("Xpo Issue links are empty");
	...
	public void downloadFile(String filename) {
		...
		XpoSftpServerTest sftp = new XpoSftpServerTest();
		sftp.downloadFile("/Inbox" + filename, downloadPath);
